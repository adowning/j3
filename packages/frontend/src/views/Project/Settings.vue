<template>
  <div class="pr-6 py-8 pl-10 h-full w-full flex flex-col">
    ...
    <header class="mt-3 flex justify-between text-textDarkest">
      <div class="text-2xl font-medium">Department Details</div>
    </header>
...
import {
  updateProject,
  getProjectWithUsersAndIssues, getProjectWithUsersAndWarehouseBoardItems
...
export default defineComponent({
  setup() {
    ...
    const { refetch } = useQuery(getProjectWithUsersAndWarehouseBoardItems, {}, () => ({
    ...
    const handleUpdateProject = async () => {
      try {
        ...
        if (res.data) {
          ...
          mutations.setProject(res.data.getProjectWithUsersAndWarehouseBoardItems)
        }
