...
module.exports = {
  ...
  async login(ctx) {
    ...
    const isValid = await strapi.plugins['users_permissions'].services.user.validatePassword(
      password,
      storedPassword
    );
  ...
  updateSettings: async ctx => {
    ...
    const usersPermService = strapi.plugins['users_permissions'].services;
    ...
    if (restrictedAccess && _.isEmpty(password)) {
      return ctx.badRequest(
        null,
        admin ? 'users_permissions.Auth.form.error.password.provide' : 'Please provide a password'
      );
    }
    ...
    if (isNewPassword && usersPermService.user.isHashed(password)) {
      ...
      return ctx.badRequest(
        null,
        admin
          ? 'users_permissions.Auth.form.error.password.format'
