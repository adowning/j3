 ...
 *
 * @description: A set of functions called "actions" of the `users_permissions` plugin.
 */
...
module.exports = {
  ...
  async createRole(ctx) {
    ...
    try {
      await strapi.plugins['users_permissions'].services.userspermissions.createRole(
        ctx.request.body
      );
  ...
  async deleteRole(ctx) {
    // Fetch public role.
    const publicRole = await strapi.query('role', 'users_permissions').findOne({ type: 'public' });
    ...
    try {
      await strapi.plugins['users_permissions'].services.userspermissions.deleteRole(
        roleID,
        publicRoleID
      );
  ...
  async getPolicies(ctx) {
    ctx.send({
      policies: _.without(
        _.keys(strapi.plugins['users_permissions'].config.policies),
        'permissions'
      ),
    });
  },
  ...
  async getRole(ctx) {
    ...
    const plugins = await strapi.plugins['users_permissions'].services.userspermissions.getPlugins(
      lang
    );
    const role = await strapi.plugins['users_permissions'].services.userspermissions.getRole(
      id,
      plugins
    );
  ...
  async getRoles(ctx) {
    try {
      const roles = await strapi.plugins['users_permissions'].services.userspermissions.getRoles();
  ...
  async searchUsers(ctx) {
    ...
    const data = await strapi.query('user', 'users_permissions').custom(searchQueries)({
      id,
    });
  ...
  async updateRole(ctx) {
    ...
    try {
      await strapi.plugins['users_permissions'].services.userspermissions.updateRole(
        roleID,
        ctx.request.body
      );
  ...
  async getAdvancedSettings(ctx) {
    ctx.send({
      settings: await strapi
        ...
        .get(),
      roles: await strapi.plugins['users_permissions'].services.userspermissions.getRoles(),
    });
  },
